  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.name" clearable placeholder="输入数据库名称搜索" style="width: 200px" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <date-range-picker v-model="query.createTime" class="date-item" />
        <rr-operation />
      </div>
      <crud-operation :permission="permission">
        <el-button slot="right" v-permission="['admin','database:add']" :disabled="!selectIndex" class="filter-item" size="mini" type="warning" icon="el-icon-upload" @click="execute">执行脚本</el-button>
      </crud-operation>
    </div>
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="530px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
        ...
        <el-form-item label="数据库地址" prop="jdbcUrl">
          <el-input v-model="form.jdbcUrl" style="width: 300px" />
          <el-button :loading="loading" type="success" style="margin-left: 7px;" @click="testConnectDatabase">测试</el-button>
        </el-form-item>
        <el-form-item label="用户" prop="userName">
          <el-input v-model="form.userName" auto-complete="off" style="width: 370px" />
        </el-form-item>
        <el-form-item label="密码" prop="pwd">
          <el-input v-model="form.pwd" auto-complete="new-password" type="password" style="width: 370px" />
        </el-form-item>
      </el-form>
...
<script>
  ...
  import crudDatabase from '../../../api/mnt/database'    //数据库相关api
  import { testDbConnect } from '../../../api/mnt/connect'    //测试数据库连接
  import eForm from './execute'   //执行sql脚本
